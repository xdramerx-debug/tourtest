...
## 0. v2 — единственное продуктовое приложение

| Что | Куда | Как |
|---|---|---|
| **Продукт `apps/scoring`** (весь функционал + 5 горячих тем) | **GitHub Pages root**: `https://xdramerx-debug.github.io/tourtest/` (`/tourtest/…`) | `deploy-pages.yml`: build scoring → `dist-pages/` + `404.html` (hash deep-link трюк D9) |
| Матрица вариантов v1 (15 сборок) | Подпуть `/variants/` (регрессионная песочница; ссылка с главной витрины-пле имеется в агрегаторе) | тот же workflow: `dist-pages/variants/{a,b,c}-d{1..5}` + индекс |
| Sync/REST сервер (`apps/server`) | как в §1 ниже (без изменений) | — |

?theme=N в query работает на всех роутах приложения (до `#/…`) и шарится ссылкой.
