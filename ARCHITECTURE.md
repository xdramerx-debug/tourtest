...
## 0. v2 — механика «одно приложение, пять горячих тем» (`apps/scoring`)

**Данные v1 подтверждены**: вся тематизируемость уже выражена CSS-переменными в `[data-design="N"][data-mode="M"]` (генерируются `scripts/build-themes.mjs` из `packages/tokens/src/themes.data.mjs`), плюс JS-паттерны экранов через `themeById[design].patterns`. Отсюда механика v2:

1. **Один бандл, пять тем**: `apps/scoring` импортирует `base.css` + **все** `theme-{1..5}.css` + `ds.css` (общая для всех с секциями `[data-design]`).
   - Бандл-эффект: темы ≈ +5×CSS-токены, но CSS мала (проверено `check:bundle`, бюджет в NFR §0); шрифты тем подгружаются браузером **лениво** (font-display: swap, запрос только при фактическом использовании семейства) — см. NFR §0.
2. **Состояние темы**: `ThemeProvider` в `@csl/app-ui` (React-state в AppContext): `{ design: DesignId, mode: Mode, setTheme(d?, m?) }`.
   - Инициализация: `?theme=N` (query, до hash) → `localStorage csl.theme` → системная тема (`prefers-color-scheme: dark` → дизайн 5 «Night», иначе 1 «Classic») → default.
   - Применение: `applyTheme(design, mode)` (tokens) ставит data-атрибуты на `<html>` — мгновенно, без перезагрузки, DOM и react-дерево не трогаются; zustand-store/роутер/открытые вводы **не размонтируются** (провайдер меняет только значение контекста).
   - Побочные эффекты `setTheme`: `localStorage['csl.theme']=design`; обновление `?theme=N` через `history.replaceState` (ссылка шарится без перезагрузки); при наличии сессии — запись `theme` в профиль клуба (`localStorage csl.club`); focus не дергается.
3. **Реактивность экранов**: все потребители `useApp()` получают новый `themeById[design]` — перерендер по месту, данные (счёт, флайты, вводы) из сторов не пересоздаются.
4. **Горячие клавиши**: `Alt+T` — следующая тема (цикл 1→5→1); `Alt+1..5` — прямой выбор; `Shift+?` (приложение) показывает help-оверлей. Не перехватываются в полях ввода (numpad/текст).
5. **Изоляция v2**: вариантные приложения v1 (`apps/variant-*`) остаются как матрица-песочница регрессии под `/variants/`; перестройка e2e на главное приложение — `e2e/scoring.spec.ts`, build/deploy — DEPLOY §0.

Тесто-ориентир: переключение 5× подряд на /play во время ввода не должно менять ни одного значения в store (unit + e2e доказательства).
